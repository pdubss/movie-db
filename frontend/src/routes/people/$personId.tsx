      ...
      <div className="flex flex-col gap-6">
        {data?.combined_credits?.cast.length && (
          ...
          >
            <AccordionItem value="item-1">
              <AccordionTrigger className="border border-white px-2">
                Actor
              </AccordionTrigger>
              {data?.combined_credits?.cast.map((credit, i) => (
                <AccordionContent
                  key={i}
                  className="flex justify-between border-b pb-0 text-balance"
                >
                  <div className="flex gap-2">
                    {" "}
                    <img src={`${IMAGE_BASE_URL}w45${credit.poster_path}`} />
                    <div>
        ...
        {data?.combined_credits?.crew.filter(
          ...
          <Accordion type="single" collapsible>
            <AccordionItem value="item-1">
              <AccordionTrigger>Director</AccordionTrigger>
